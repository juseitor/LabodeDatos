...
Consulta1_auxiliar = """
    SELECT DISTINCT ee.Provincia, ee.Departamento, ep.Provincia, ep.Departamento
    FROM EE_limpio AS ee
    LEFT OUTER JOIN EP_limpio AS ep
    ON ep.Provincia = ee.Provincia AND ee.Departamento = ep.Departamento
    GROUP BY ee.Provincia, ee.Departamento, ep.Provincia, ep.Departamento
"""
consulta1_auxiliar = db.query(Consulta1_auxiliar).df()
#%%
Consulta2_auxiliar = """
    SELECT DISTINCT ee.Provincia, ee.Departamento, ep.Provincia, ep.Departamento
    FROM EE_limpio AS ee
    RIGHT OUTER JOIN EP_limpio AS ep
    ON ep.Provincia = ee.Provincia AND ee.Departamento = ep.Departamento
    GROUP BY ee.Provincia, ee.Departamento, ep.Provincia, ep.Departamento
"""
consulta2_auxiliar = db.query(Consulta2_auxiliar).df()
...
# Cambiamos los acentos de las provincias 
...
acentos_provincia =  """
            ...
            """
EP_df = db.query(acentos_provincia).df()



#%%
Consulta_dif_Departamento_departamento = """
    SELECT DISTINCT ee.Departamento, ep.departamento
    FROM EE_df AS ee
    LEFT OUTER JOIN EP_df AS ep
    ON ep.provincia = ee.Jurisdicción AND ep.departamento = ee.Departamento
    GROUP BY ee.Departamento, ep.departamento
"""

consulta_dif_Departamento_departamento = db.query(Consulta_dif_Departamento_departamento).df()
